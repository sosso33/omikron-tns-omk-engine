...
| todo/handoff-shoot-mode.md | ~5k | the SHOOT MODE task, handed off 2026-09-10: **how to reach a shoot phase in one command** (`--area 230 --scene-chunk 56`), what a person has confirmed against what is only measured, the FIRING that was connected the same day (the trigger through `MDSHOOT0` to a bolt that flies and stops at walls - and the hit, read and not ported), and ten traps - the first being a harness that did something the real path did not |
