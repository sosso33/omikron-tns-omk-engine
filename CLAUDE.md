...
| todo/handoff-vita-port.md | ~3k | **read FIRST to pick up the PS VITA**: where the port stands (builds, runs on a console to the start menu, plays in Vita3K), the build / emulator / console-layout recipes, what waits on the reader (the IAM re-copy in BINARY mode - FileZilla's ASCII mode was the black screen - and the hardware films), the next steps in order, and the Vita traps. `todo/vita-port.md` is the plan and full record |
| todo/handoff-shoot-mode.md | ~5k | the SHOOT MODE task, rewritten 2026-09-11 after the gunmen learned to FIGHT: **how to reach a shoot phase in one command** (`--area 230 --scene-chunk 56`), every step's state with its commit (the gunmen's fire, aim, fall, walk, wall test, push, steering and actions, and the player's death), what a person has confirmed against what is only measured or not yet played, **what is left** (the hurt reaction next, the patrol, robber 77's turn loop, a lost guard, the owed sweep), sixteen traps - the first being a harness that did something the real path did not - and the instruments, `--shoot-health` among them |
...
| todo/slider.md | ~30k | the SLIDER (next-tasks 16): the sneak's transport, the flight model, how a ride is entered and ends (`MDACTION`'s arm, the door side, the two door clips bound by name), the seat, the journey in and across cities, the release. PLAYED AND CONFIRMED 2026-09-08 after three rounds of play reports; read its two "played" sections for what a headless log cannot see |
...
| todo/multiplan.md | ~3k | MULTIPLAN (screen 2; next-tasks 12), the storage kiosk: the 82 sites, the headless route (the pharmacy's zone 884), the widget tree, the open / button hook / row arms read from the raw image, and six steps |
| todo/sneak.md | ~6k | the SNEAK device (screen 9): what works, the closed questions (row scrolling, `Utiliser sur`, the empty memory page), and §5 - next-tasks 10, the IDENTITY / OPTIONS / QUIT pages: the three draw hooks and the event-44 property pairs behind the identity text, screen 35 hosted by the Options tab, the quit request, the headless route and seven steps |
| todo/shops.md | ~3k | the SHOP screens (20..28, 32; next-tasks 11): how one is reached headlessly (the pharmacy, AREA 39), what the port draws today, `Ui_OpenShop` and the buy/sell arms read from the raw image, and six steps |
| todo/handoff-fight-mode.md | ~5k | **read FIRST to pick up MELEE**: how to reach a real fight in one command (`--fight-supermarket`, the chunk's own script, no harness), what steps 0-4 landed with their commits, the ONE open fault (the camera never comes back after a fight - with what is established and what was already refuted), the bindings gap, and six traps including `grep` going binary-quiet on the viewer's log |
...
| the actor runtime | `ACTOR_STATE` 0..17 mapped **and run** - 14 is the WATER state (`RSTNAGE`/`MDDIVEND` write it), 7 and 8 are the mount and the ride of one slider (`MDSLIDOU` refuses to dismount from anything but 8), and 7 has no case in `Actors_TickAll` at all; the walker (30° slope, 30 cm step, fall tiers), **the player's VERTICAL** - the walk's float was this port's own (the anchor latched from a pose and the drop measured from another origin; the clips' pelvis drop and leg lift cancel to 0.03) and the JUMP is `MDJUMP0A`/`01`: 2.5 m forward along -Z over `N = entry+12` frames with `+220 = -g*N/2`, the ballistic launch, measured at 14 frames and 22.9 cm of lift - a flat LEAP; `MDJUMP03` bands the landing 5.00/3.00/1.50 m into ACTOR_STATE 18, and `dword_6A52CC` is what turns the ground probe's absorption off for the jump's duration. combat block fully decoded, `tab_special_move` (66 rows) - four of whose handlers are themselves `ACTOR_STATE` writers - camera-mode presets |
...
| the interface text / save directory | `IAM\<Screen>` + `IAM\FRENCH\`; `IAM\GAMES` = 3496 + 256x32808; the 256x72 directory record **established** from `SaveDir_Build` — and its fourth field read 2026-09-06: **slot +108**, the player record's `+8`, the CHARACTER'S NAME, which is what the load panel labels a row with. It was recorded as "+76, not a string"; the wrong offset lands in the DB's array counts, and the negative result was the offset's, not the field's |
| **saving and loading, end to end** | the WRITER (`Game_WriteSave`'s four copies, `SaveDir_ClearSlot`'s one byte, `sub_4092A0`'s create arm, the 128x96 X1R5G5B5 thumbnail); the load, which is a REQUEST served between script pumps and covered by AREA 118's own GRID sequence; and the panel — rows, wrapping selection, profile wheel, selection box, connector, picture, the two confirms. **A save costs one *anneau*** and is refused twice over, by the save point's script and by the panel. All of it PLAYED: load, save, overwrite, delete (`docs/GAME_STATE.md` §8b/8c, `docs/UI.md` §3g) |
| the **I2D 2D layer** | the 16-layer display list, its 7 primitives and their pools (**plus 2 recorded as uncalled until 2026-09-15** - both are called from table-dword draw hooks: the save thumbnail and the monitors' INTERFERENCE - without whose pool the 4862-node cap does not add up), the DirectDraw colour-key back end, the bitmap cache, the four flag families. The per-layer cache is a **HEAD** cache, so inside one layer the rest draw in REVERSE submission order; and the blits do not test the source Y |
...
| the **fight AI** (`.CTL` `+76`/`+80`) | 4 profiles of button combos injected into the player's own input queue; harder = shorter wait; input-bit union 0xCFF. **Each slot also carries a PERCENTAGE** (`{weight, count, moves}`), which is what every `Fight_TickAI` choice rolls against, and the AI presses **eight more moves compiled into the exe** (`tables/fight_ai_moves.json`). MELEE RUNS since 2026-09-16 - `engine/src/actor/fight.*`, `verify.py: engine: melee` |
