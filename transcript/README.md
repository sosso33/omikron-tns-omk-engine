...
| `session-2026-09-06_5878c3a9` | 2026-09-05 12:33 → 2026-09-06 03:54 | **the graphical options, the `.3DO` light table, and a bug that took three wrong answers before it took the right one.** The long first half is steady: the save file's 3496-byte header decoded field by field and found to BE the settings block, so saving a game saves your options; the clip distance traced to the four things one option sizes; the sky read as a painted ceiling rather than a dome; the fog read as linear and BLACK. Two things in it are worth more than the findings. A reader challenged **"the world unit is an inch"** as implausible for a French studio, which is the right challenge and forced the claim to be re-grounded on a MEASUREMENT (a model 70.85 units tall against a 1.80 m man, and the engine's own `"Hauteur : %f meter"` debug string dividing by 0.0254) rather than on a constant found in the code - and the reader then supplied the explanation the measurement could not, a 3D Studio pipeline, from an old job advertisement. And the reader supplied the 1999 official spec sheet, whose *Multilights* line pointed at **4179 light records in 216 models that nobody had read**, and whose claimed BSP tree was then refuted from inside the format by accounting for 99.9986% of every `.3DO` byte. **The second half is the one to read.** A held ENTER counting as several presses was fixed, published, and the fix was wrong in a way that took a fourth session-day to see: it installed the TAKE bank on any successful press, so finishing a shop seller's conversation played a reach-wait-put-back on nothing. Chasing that produced, in order, a correct transcription fix that changed nothing observable, a correct wiring change that made the viewer WORSE (the action button died after every conversation, because `Perso_SetInputEnabled` has its sense backwards and the port believed the rename), and only then the actual cause. Read it for how often the confident answer was about the TOOLING: `--hold`'s comma-versus-plus syntax made every "second press" test press nothing and published two false conclusions; `--frames --dump` opens a real window, so a reader who saw one appear reasonably walked the player and two comparison renders measured his walk; and zsh word-splitting struck four times in the same session, the last inside a shell function, turning a three-way comparison into three zeros that read as "the fix broke everything". |
