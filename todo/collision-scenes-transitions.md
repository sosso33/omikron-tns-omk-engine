...
| 3c | scene objects' collision in the sweep (`o3de_ForEachMeshInBox` walks every linked mesh; the port sweeps the decor soup only) — the tunnel's doors are scene objects; the airlock has none | **done 2026-09-05, and the premise was half wrong**: the tunnel's doors (`T01door01/02`) and the restaurant's (`Ported26`/`Porteg26`) are SET MESHES moved by `Script_MoveObjectOnPath`, so the viewer's motion patch already carried their faces into the soups - for the ACTIVE pool. The real gap: a transition's door program runs in the OUTGOING pool (`sceneOut()`), whose motions the viewer never read, so a door closing behind the player neither drew nor collided. The mesh patch now merges both pools' motions and node scales, and logs `motion: mesh ... moved by the OUTGOING pool` |
| 3d | a check that walks a transition headless and asserts the door resolves and plays, and the outgoing set survives until the feet are on the destination | **done**: `engine: tunnel door walk` stands the walker at x 7300 in the tunnel facing its west door and holds forward: the zone carries doors 3/4, door 3 opens by the active pool, arrive state 6, feet on AREA 0, door 4 closes by the OUTGOING pool, the tunnel hidden after, and he ends past the door with no `NO FLOOR` |
| 3f | **the restaurant's void** (2026-09-05): leaving ARESTO the reader fell for ever from (6717, 0, 593). Read: not the transition — the doors resolved (handle 10 = `Porte26open`, 11 = `Porte26close`, resto.SCX's objects 0/1) and the completion hid Anekbah correctly, since his feet never reached its decor. He stepped off the dining floor's south edge (a 31.5 drop onto a sunken ledge, which the headless walk lands) and the port carried the WALK's horizontal delta through the air, drifting him off the ledge over a hole in the geometry. `Actor_ApplyMotion` moves the actor by +216/+224 × dt, velocity fields the fall state never writes (issue 68: "falling mainly on a single axis"); the walker's airborne step no longer applies the walk's delta. **Done.** The "floorless strip under the doorway" was a measurement at the WRONG z: it was taken at z 593, the south edge, where the ledge is. The doorway is at z 400 (AREA 46's zones 960/975 and AREA 0's zone 100 centre there, the door leaves `Ported26`/`Porteg26` at z 373/427 sliding apart), and there `wall_probe` finds Anekbah's floor to x 6666 and the restaurant's from x 6676 at the same height (y 0) - a 10-unit OVERLAP, no gap. **Closed** |
| 3e | the CRATE CORNER: the reader was held between the alley's wall and the moved crates at the exit (7799, −78.7, 2895: "NO FLOOR ahead, 1 wall pass hit, ground under him" every frame). The slide's remainder points off the floor's edge and the probe reverts it, so nothing moves; the engine's accumulated blocked-direction mask (the last unported part of `Actor_Move`) is the candidate. The viewer prints `walker: STUCK` with the position when it happens | **investigated, no port fault found**: reproduced headless (`--area 142 --stand 7795,-79,2890,146 --hold '0*1,k200*80'`). The spot is the sas floor's DIAGONAL EDGE (`Icouloir`'s triangle (7719,2860)-(7822,2905)-(7765,3027), 37 walkable triangles in the whole set) under a crate mesh `IMcube010` (flags 0x4 - a collider by `Sweep_MeshTest`'s own rule, which skips only 0x41 and 0x20000000) whose lower face hangs 58 units above the floor, so Kay'l's 70-unit capsule has its head inside it. The sweep finds the body already touching, pushes out along the crate's side, and the remainder runs along the floor's edge into the void, which the port reverts ("NO FLOOR ahead"). `Walk_ProbeGround` (0x00467030 → `sub_443560`/`sub_443810`, read) is a vertical ray at the feet through the meshes in a body-radius box, so the engine's probe finds no floor there either; what its response does with that answer is unread. Nothing read says the engine passes where the port holds; a play comparison in the original at the airlock exit is what would settle it. Tools left for it: `mesh_list`, `soup_dump`, `scx_paths` |
