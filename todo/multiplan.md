...
| 3 | **the TRANSFERS**: case 36 requests 7 and 8 in the channel (read them whole first), messages 6/7/8, the re-bind and the focus rule. Check: an object crosses each way and the refusals post their message | **done 2026-09-15** — the row callback `0x004B05C0` is recorded by the walk (`takeMultiplan`: button 0 -> request 7, 1 -> 8, a row with no tag refuses) and carried out by the viewer as case 36 does: 7 refuses kind 1 and a full storage, else front-inserts into list 1 and removes from 0; 8 refuses a full sneak, else runs the session's `Inventory_Insert` ladder (`insertArm`: kinds 12/13 apply their effect and take no row, 2..11 merge into a related row) and removes from list 1 whatever it answered. Message 8 / 7 / 6 through the header for 5000 ms; an emptied list hands the focus to the buttons. **The header hook re-read whole**: on the Examiner page or with the rows focused it fetches the selected row's name and never prints it (`"%s"` at `0x004E5AF8`, the label its only argument). **A latent viewer bug found**: `Session::dataRoot_` came only from `loadTraffic`, which `--no-crowd` skips, so every object kind read -1 and a withdrawn ring took a sneak row - `setDataRoot` now runs unconditionally. Not modelled: `Inventory_Insert`'s gun arm that also swallows loose ammunition of kind + 5 (the session's ladder answers Row there, as it did before). `verify.py: engine: multiplan transfers` - deposit 171 + withdraw 163 (rings 2 -> 7, no row); a Waver refused (message 7); an 18-slot sneak refused (message 6). Mutations, each red with its own output (`df32519`): the kind-1 gate dropped (the Waver deposited, message 8); `setDataRoot` dropped (163 takes a `row`, rings stay 2); requests 7 and 8 swapped in the walk (171 withdrawn from the sneak into itself, three times); the storage removal skipped on a withdrawal (163 stays in the kiosk) |
| 4 | **EXAMINER and DÉTRUIRE**: the examine page `0x004E5998` (`sub_42B420`, and what it draws), the destroy confirm `0x004E5A00` (request 6, message 4). Check: the page shows the object, a destroyable object goes, a protected one refuses | **done 2026-09-15** — the row callback's arm 2 installs the page (text scroll reset, the builder's first instruction) and records request 4, which the viewer posts as world **message 4** with the object: `sub_42B420(tag, 4)` is event 30 then 43, whose sender is `ObjectSlot_Id` of case 30's slot. It is SUBSCRIBED - the GLOBAL table handles it - so an examine runs a script. The page's box is draw hook `0x004780A0`, the sneak's examine box, and the composer now keys the text and model on that hook rather than the sneak's list address: every arm ends in `sub_477F60` (the description, event 40's `+8`), kind 15 draws only that, 16 the `IMAGES` bitmap first, anything else case 30's model first. Arm 3 installs the confirm (no tag test); its second line `0x004B0C30` is the selected row's name; Oui `0x004B08C0` refuses with message 4 unless the source is the kiosk, else request 6 (`flags & 2` destroys, silently; otherwise message 4), then the kiosk again on its rows; Non only reinstalls. **Found, and FIXED as its own task the same day**: the SNEAK's Examiner (`0x0049BFF0`, the call at `0x0049C034`) also calls `sub_42B420(tag, 4)`, and the port had never posted message 4 there - the same global handler went unrun. Now posted on entering the page (`UiListState::pendingExamineMessage`, `verify.py: engine: sneak examine message`). `0x0049BBF0` and `0x004B0410`, two more bodies of that shape, are unreferenced in the image. `verify.py: engine: multiplan examine` - Examiner on 176 (text only, message 4 by the global table); 176 destroyed; the rings refused. Mutations, each red with its own output (`646d407`): request 6 testing flag `0x20` (176 refused, the rings destroyed); the source test never passing (both refused as "not the kiosk's list"); message 4 not posted ("no resident chunk subscribes"). **Not covered by the check**: the composer's keying of the text and model on draw hook `0x004780A0` - it is seen in a render (the notice's description laid out in the page's box), not asserted |
| 5 | **the CLOSE and the answer**: event 26, variable 19 at the 13 sites that keep it | **done 2026-09-15** — the close `0x004B02D0` is `Game_RaiseEvent(26, 0)` then the generic close; the viewer's close-without-answer path raised event 26 only for the shops and now does for screen 2 too (the open list 0 -> -1). No instruction on screen 2 writes `dword_930750` (the writers near it, `0x004B008A` / `0x004B013B`, are the neighbouring screens' - `0x004B00B0` installs the LIFT's panel), so every site resumes with the preset **-1**. Of the 82 sites **13 keep it in variable 19, and none of them reads it**: each runs on into `camera.set`, `fade.from_color` or `end` (AREA 178 x5, 179 x5, 237, 253, SCENE 51); 69 discard it, the pharmacy's (AREA 39) among them. `verify.py: engine: multiplan close` - open, TAB: the -1 resume and event 26. Mutation (`6175cdb`): screen 2 dropped from the event-26 arm - the close line is missing and the check goes red |
| 6 | **PLAY** - a reader's pass; `todo/play-test.md` gets the recipe | **CONFIRMED IN PLAY 2026-09-15** - the reader: *"Ok, i tested it, no issues"*. The recipe stays in `todo/play-test.md` 13 |
