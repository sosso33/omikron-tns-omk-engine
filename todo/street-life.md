...
| 4 | draw them: `omk-play` stages the pedestrian pool like the extras (shared `CharModel` per model name, the walk clip at each walker's own phase and speed); add `--area N --address A` so a person can stand in a street; **watch Anekbah and Jaunpur**, file what is seen in `todo/omk-play.md` | **done 2026-09-03, WAITING TO BE WATCHED** — `omk-play --save traces/save-appart.bin --area 0 --stand 1804,0,-6890,336` (or `--address A`; `--density`, `--no-crowd`); the walkers draw, `engine: street frame`; the crowd models' four LOD skeletons found and cut, for the extras too |
| 5 | the crowd push: read `sub_45E390`/`sub_45E690`/`sub_45DFF0`/`sub_438040`, port `SpatialIndex` into the player's tick; invariant over a walk into a body, not a still frame | **done 2026-09-03** — `actor/spatial.*` (the two tests, the reach box), `Session::crowdPush`/`talkToPedestrian` (messages 15/16 and 13/14 through `postMessage`), `PlayerController::nudge`, the viewer hook before the tick; `engine: crowd push` (shape, a walk into a walker, a talk) |
| 6 | confirm the interaction path in play (zone lines, head look-at); docs: RECONSTRUCTION log row, CLAUDE.md §4 row, `engine/README.md` coverage row | **done 2026-09-03** — the interaction path confirmed HEADLESS (the bump/talk/zone scripts run; their voices are among the 551 VOICEOFF entries that do not ship), the head look ported (`aimHead`, `engine: head look`), CLAUDE.md §4 row, engine/README coverage; the play confirmations listed in STREET_LIFE 'Still to be watched' |
...
| date | what |
|---|---|
| 2026-09-03 | Step 6. The passer-by voices cannot be heard in this data set - 17 of 561 VOICEOFF files ship - so 'confirm the interaction path' became: the scripts run, the messages post, the camera shakes; the audio is a property of the data. The head look was the one unported piece left and is now a pose-space aim with the engine's clamps and ease, measured on the Demon's forward. |
| 2026-09-03 | Step 5. The push is two tests: spheres for actors, an ellipse two radii long for walkers with a quarter of the penetration applied - and the reach box clips the ellipse at one radius, which looked like a bug in the probe until the box was remembered. The bump and talk messages post through the Session's existing message path; the talk only reaches a walker standing at an action point. |
| 2026-09-03 | Step 4. The first frame showed a T-posed figure over the crowd: a crowd model is four LOD skeletons in one file and only the first is posed. Cut to the skeleton the tracks name - and the same cut fixed the twenty authored extras of Anekbah, which wear the same models. The street start needs a SAVE for the player record and a camera request (`Camera Player`), since no script runs the hand-over. To watch: the pace, the facing over a turn, the following distance. |
