...
| 3 | draw them in `omk-play`, then WATCH a city street | **done 2026-09-04** — `VehStaged` beside `PedStaged` (a vehicle is RIGID: composed once, turned and translated per frame, placed at `sub_437F80`'s `y - 30.75`), `engine: traffic frame`; a moto and a hover taxi WATCHED crossing Anekbah. The drawing found a latent crowd bug — see the log |
...
| date | what |
|---|---|
| 2026-09-04 | Step 3, and it earned its place. Nothing drew, twice over. First I could not aim: the vehicles were staged with correct coordinates and simply out of frame, and three renders went into hunting them before I printed the bearing instead of guessing a yaw. Then the real fault: `play.cpp` evicts from `charModels` every frame any model no STAGED ACTOR wears, and the circuit's bodies are not staged actors - so the traffic drew on frames 0 and 1 and then vanished, its cached `CharModel*` pointing at a freed map node. **The crowd has the same bug and is only masked**, because a city's authored extras wear the same PERSOS models and hold them resident by accident. Fixed for both. And the first version of the frame check did NOT catch it: reading one report at frame 0, it passed on the broken build (873 pixels still differed). Reading all three reports and raising the threshold, the mutation drops the leanest to 0 drawn and 524 pixels. A check written against the fault it was made for, and shown to fail on it - PORTING B2, the hard way. |
