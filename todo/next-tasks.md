...
| 3 | 9 | main menu completed - **SURVEYED 2026-09-22, `todo/start-menu.md`**: both items descend, OPTIONS draws only its heading (its panel has one item and no hook - the rows are native) and QUIT's confirm draws but neither button acts. Neither half is in the widget records, so the next step is a READING, not a wiring | the widget tree and the answer sites are lifted, the start menu already answers for itself, and the save/load menus built for item 8 are most of the machinery. Pin what "new game is not correct" means in one run before building anything |
...
| 10 | 17 | fight mode | **STARTED 2026-09-16**, `todo/fight-mode.md` (step 0, the reading, is done). Large, and data-constrained by construction: `fight.begin` announces nothing, so no trace can ever be its oracle |
...
| 7 | missing animations (lift doors, Kay'l's drawer) | **FIXED** | strong | the path is a DISPLACEMENT, not a position: `node = sample(t) - sample(t0) + anchor`. The flat's doors now slide 87.2 down, close behind you, and are AUDIBLE (gain 0.03 -> 1.00, the same fault reaching the 3D sound). The lift's were right by accident. Door COLLISION: it FOLLOWS the mesh (`patchSoup`), and 2026-09-09 proved it the hard way - a shop's leaves that never moved kept their 24 triangles across the doorway and shut the player in (`todo/omk-play.md` 94) |
...
| 13 | any remaining UI screens | **M** | fair | **STARTED 2026-09-17**, `todo/missing-ui.md`: every `ui.open` site counted (236 over 24 screens), and the LIFT - the reader's own example - FIXED. It opened all along; what stopped it was the zone scan, which had no height filter, so one press activated five stacked lift scripts and the answer went to the wrong one. The FIGHT SIM opens and needs its keypad's three callbacks read |
...
| 17 | fight mode | **L** | good | **STARTED 2026-09-16**, `todo/fight-mode.md`: the reading is done and it closed two questions - op 62's third field is the AI LEVEL, and the difficulty is ADAPTIVE (`Niveau Combat`, 36/36/36 over the corpus, profile 4 unreachable). The AI profiles and combat block were already read; nothing is wired yet |
